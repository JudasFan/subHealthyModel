    ...
    class warning:
        ...
        def __init__(self, wtgs_path, start_time, end_time, author):
            ...
            for key in self.tag_set.keys():  # select the english name of tag based on matching
                if '发电机' in key and '轴承温度' in key and ('DE端' in key or '驱动端' in key) and ('N' not in key and '非' not in key):
        ...
        def run(self):
            ...
            AbnormalRecord = pd.DataFrame(AbnormalRecord,columns=['farm_code', 'farm_name', 'wtgs_id', 'model_name', 'abnormal_time','generator_DE_temperature', 'avg_temperature', 'avg_power'])
        ...
        def export(self):
            ...
            if len(self.abnormal_records) > 0:
                ...
                pd.io.sql.to_sql(self.abnormal_detail, 'GenDETemp', con=conn, if_exists='append')
                conn.close()
                try:
...
class query:
    ...
    def curve(self,abnormal_record):
        ...
        (conn,cur)=sqlite_conn()
        sqlstr = 'SELECT * FROM GenDETemp WHERE wtgs_id=\''+abnormal_record['wtgs_id'].iloc[0]+'\' AND abnormal_time BETWEEN \'' + abnormal_record['abnormal_start_time'].iloc[0] + '\' AND \'' + abnormal_record['abnormal_end_time'].iloc[0]+ '\''
        res = pd.read_sql(sqlstr, con=conn)
        ...
        res['generator_DE_temperature'].plot(ax=ax1,color='r')
        res['avg_temperature'].plot(ax=ax1)
        res['avg_power'].plot(ax=ax2,color='g')
        ax1.legend(['generator_DE_temperature','avg_temperature'], loc='upper left')
...
if __name__=="__main__":
    ...
    # aaa = query('2017-10-14 17:58:00', '2017-10-14 19:56:00')
    # res=[]
    # for id in range(20004001,20004025):
    #     if id!=20004003:
    #         res.append(['广西恭城2.0',str(id),'2017-10-14 17:58:00','2017-10-14 19:56:00','2.0'])
    # res=pd.DataFrame(res,columns=['farm_name','wtgs_id','abnormal_start_time','abnormal_end_time','abnormal_duration'])
    # for i in range(len(res)):
    #     print(res.ix[i:i])
    #     aaa.detail(res.ix[i:i])
    #     # aaa.curve(res.ix[i:i])
