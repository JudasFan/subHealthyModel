...
class generate:
    ...
    class warning:
        ...
        def run(self):
            ...
            AbnormalRecord = pd.DataFrame(AbnormalRecord,columns=['farm_code', 'farm_name', 'wtgs_id', 'model_name', 'abnormal_time','gearbox_NDE_temperature', 'avg_temperature', 'avg_power'])
        ...
        def export(self):
            ...
            if len(self.abnormal_records) > 0:
                ...
                pd.io.sql.to_sql(self.abnormal_detail, 'GearboxNDETemp', con=conn, if_exists='append')
                conn.close()
                try:
...
class query:
    ...
    def detail(self,abnormal_record):
        ...
        res=pd.read_sql(sqlstr,con=conn)
        res.to_csv("D:/work/亚健康模型/1.5&2.0MW/10.04-10.10/" + abnormal_record['farm_name'].iloc[0] + '-' + str(
    ...
    def curve(self,abnormal_record):
        ...
        (conn,cur)=sqlite_conn()
        sqlstr = 'SELECT * FROM GearboxNDETemp WHERE wtgs_id=\''+abnormal_record['wtgs_id'].iloc[0]+'\' AND abnormal_time BETWEEN \'' + abnormal_record['abnormal_start_time'].iloc[0] + '\' AND \'' + abnormal_record['abnormal_end_time'].iloc[0]+ '\''
        res = pd.read_sql(sqlstr, con=conn)
        ...
        res['gearbox_NDE_temperature'].plot(ax=ax1,color='r')
        res['avg_temperature'].plot(ax=ax1)
        res['avg_power'].plot(ax=ax2,color='g')
        ax1.legend(['gearbox_NDE_temperature','avg_temperature'], loc='upper left')
        ...
        # plt.show()
        plt.savefig("D:/work/亚健康模型/1.5&2.0MW/10.04-10.10/"+abnormal_record['farm_name'].iloc[0]+'-'+str(abnormal_record['wtgs_id'].iloc[0])+'-'+MODEL_NAME+'-'+str(round(float(abnormal_record['abnormal_duration'].iloc[0]),2))+".png")
...
if __name__=="__main__":
    #aaa = generate('2017-11-15 00:00:00', '2017-11-22 00:00:00', '刘利军')
