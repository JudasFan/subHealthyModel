...
class generate:
    ...
    class warning:
        ...
        def run(self):
            ...
            AbnormalRecord = pd.DataFrame(AbnormalRecord,columns=['farm_code', 'farm_name', 'wtgs_id', 'model_name', 'abnormal_time','gearbox_oil_temperature', 'avg_temperature', 'avg_power'])
        ...
        def export(self):
            ...
            if len(self.abnormal_records) > 0:
                ...
                pd.io.sql.to_sql(self.abnormal_detail, 'GearboxOiTemp', con=conn, if_exists='append')
                conn.close()
                try:
...
class query:
    ...
    def curve(self,abnormal_record):
        ...
        (conn,cur)=sqlite_conn()
        sqlstr = 'SELECT * FROM GearboxOiTemp WHERE wtgs_id=\''+abnormal_record['wtgs_id'].iloc[0]+'\' AND abnormal_time BETWEEN \'' + abnormal_record['abnormal_start_time'].iloc[0] + '\' AND \'' + abnormal_record['abnormal_end_time'].iloc[0]+ '\''
        res = pd.read_sql(sqlstr, con=conn)
        ...
        res['gearbox_oil_temperature'].plot(ax=ax1,color='r')
        res['avg_temperature'].plot(ax=ax1)
        res['avg_power'].plot(ax=ax2,color='g')
        ax1.legend(['gearbox_oil_temperature','avg_temperature'], loc='upper left')
