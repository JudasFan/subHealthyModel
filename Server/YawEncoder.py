    ...
    class warning:
        ...
        def __init__(self,db_path,start_time,end_time,author):
            ...
            for key in self.tag_set.keys():
                if '机舱' in key and '角度' in key:
                    self.nacelle_position = self.tag_set[key]
                if '运行状态' in key:
            ...
            if len(wtgsValue)>0:
                for record in wtgsValue:#逐机组循环
                    ...
                    while rowi < len(record):
                        if record[self.run_condition].iloc[rowi] in [3, 9]:
                            rowj=rowi+1
                            if rowj >= len(record):
                                break
                            else:
                                if (record['real_time'].iloc[rowj] != record['real_time'].iloc[rowj-1] and abs(record[self.nacelle_position].iloc[rowj] - record[self.nacelle_position].iloc[rowj-1]) / (
                                    record['real_time'].iloc[rowj] - record['real_time'].iloc[rowj-1]).seconds >= 20) and \
                                        not ((record[self.nacelle_position].iloc[rowj] in [358, 0] and record[self.nacelle_position].iloc[rowj-1] in [358, 0]) or (
                                                record[self.nacelle_position].iloc[rowj] in [359, 0, 1] and record[self.nacelle_position].iloc[rowj-1] in [359, 0, 1]) or (
                                                        record[self.nacelle_position].iloc[rowj] in [360, 0, 1, 2] and record[self.nacelle_position].iloc[rowj-1] in [360, 0,1,2])):  # 变化率绝对值大于等300度/s，异常开始时刻
                                    ...
                                    if (et - st).days == 0:
                                        ...
                                        while rowj < len(record):
                                            if (record['real_time'].iloc[rowj] != record['real_time'].iloc[rowj-1] and abs(record[self.nacelle_position].iloc[rowj] - record[self.nacelle_position].iloc[rowj-1]) / (
                                                record['real_time'].iloc[rowj] - record['real_time'].iloc[rowj-1]).seconds >= 20) and not ((record[self.nacelle_position].iloc[rowj] in [358, 0] and record[self.nacelle_position].iloc[rowj-1] in [358,0]) or (record[self.nacelle_position].iloc[rowj] in [359, 0, 1]
                                                    and record[self.nacelle_position].iloc[rowj-1] in [359, 0, 1]) or (record[self.nacelle_position].iloc[rowj] in [360, 0, 1, 2] and record[self.nacelle_position].iloc[rowj-1] in [360,0,1,2])):
                                                st = datetime.datetime.strptime(str(record['real_time'].iloc[rowi]),"%Y-%m-%d %H:%M:%S")
                                                et = datetime.datetime.strptime(str(record['real_time'].iloc[rowj]),"%Y-%m-%d %H:%M:%S")
                                                ...
                                                if record[self.nacelle_position].iloc[rowj] == record[self.nacelle_position].iloc[rowj-1]:
                                                    rowj += 1
                                                else:
                                                    break
                                rowi = rowj
        ...
        def query_real_data(self, path, q):
            ...
            query_field += ',' + self.nacelle_position

            query_condition = self.run_condition + ' is not null '
            query_condition += 'AND ' + self.nacelle_position + ' is not null '
