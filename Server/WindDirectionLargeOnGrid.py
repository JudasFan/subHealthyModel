...
# @Author  : liulijun
# @Site    : 
# @File    : WindDirectionLargeOnGrid.py
# @Software: PyCharm
    ...
    class warning:
        ...
        def __init__(self,db_path,start_time,end_time,author):
            ...
            for key in self.tag_set.keys():
                if int(int(db_path['FARM_CODE'].iloc[0]) / 10000) == 1:
                    self.wind_angle = 'grVaneDirection'
                else:
                    self.wind_angle = 'iVaneDiiection'
                if '运行状态' in key:
            ...
            if len(wtgsValue)>0:
                for record in wtgsValue:#逐机组循环
                    ...
                    while rowi < len(record):
                        if abs(record[self.wind_angle].iloc[rowi]) >= 30 and record[self.run_condition].iloc[rowi] == 3:  # 风向角大于30度且处于并网状态
                            rowj = rowi + 1
                            if rowj >= len(record):
                                break
                            while rowj < len(record):
                                if abs(record[self.wind_angle].iloc[rowj]) >= 30 and record[self.run_condition].iloc[rowj] == 3:  # 风向角大于30度且处于并网状态
                                    if rowj == len(record) - 1:  # 假设到统计周期末尾均满足条件（放置进入死循环）
                                        ...
                                        if (et - st).seconds >= 1800 and rowj >= rowi + 900 and record[self.run_condition].iloc[rowj] == 3:
                                    ...
                                    et = datetime.datetime.strptime(str(record['real_time'].iloc[rowj]),"%Y-%m-%d %H:%M:%S")
                                    abduration = (et - st).seconds / 3600
                                    if (et - st).seconds >= 1801 and rowj >= rowi + 900 and record['real_time'].iloc[rowj] != 3:
                                        self.abnormal_records.append([farmcode, farmname, wtgsid, wtgsbd, MODEL_NAME,st.strftime("%Y-%m-%d %H:%M:%S"),et.strftime("%Y-%m-%d %H:%M:%S"), abduration,self.author, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
                                    rowi = rowj + 1
                                    break
                        else:
        ...
        def query_real_data(self, path, q):
            ...
            query_field = self.wind_angle
            query_field += ',' + self.run_condition

            query_condition = self.wind_angle + ' is not null '
            query_condition += 'AND ' + self.run_condition + ' is not null '
