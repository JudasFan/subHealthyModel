    ...
    class warning:
        ...
        def __init__(self, wtgs_path, start_time, end_time, author):
            ...
            for key in self.tag_set.keys():  # select the english name of tag based on matching
                if '发电机' in key and '温度' in key and 'U' in key:
                    self.U_temp_tag = self.tag_set[key]
                if '发电机' in key and '温度' in key and 'V' in key:
                    self.V_temp_tag = self.tag_set[key]
                if '发电机' in key and '温度' in key and 'W' in key:
                    self.W_temp_tag = self.tag_set[key]
                if '功率' in key:
        ...
        def run(self):
            ...
            q2 = manager.Queue()  # hold the value of U_temp
            q3 = manager.Queue()  # hold the value of V_temp
            q4 = manager.Queue()  # hold the value of W_temp
            ...
            for index, row in self.wtgs_path.iterrows():
                ...
                result = p.apply_async(self.query_temp_mins_value, args=(wtgs, q1, q2, q3, q4))
            p.close()
            p.join()
            ...
            self.UTempValue = pd.DataFrame()
            self.VTempValue = pd.DataFrame()
            self.WTempValue = pd.DataFrame()
            while not q1.empty():
            ...
            while not q2.empty():
                if self.UTempValue.empty:
                    self.UTempValue = q2.get()
                else:
                    self.UTempValue = self.UTempValue.join(q2.get())
            while not q3.empty():
                if self.VTempValue.empty:
                    self.VTempValue = q3.get()
                else:
                    self.VTempValue = self.VTempValue.join(q3.get())
            while not q4.empty():
                if self.WTempValue.empty:
                    self.WTempValue = q4.get()
                else:
                    self.WTempValue = self.WTempValue.join(q4.get())
            ...
            for TempValue in [self.UTempValue,self.VTempValue,self.WTempValue]:
                new_abres_detail,new_abres_info=self.uvw_abnormal(TempValue)
        ...
        def uvw_abnormal(self,TempValue):
            AbnormalRecord = []
            ...
            for abnormal_wtgs in abnormal_wtgs_list:  # loop all possible wtgs
                ...
                if len(abnormal_wtgs_record) >= MEAN / MINUTES:
                    rowi = 0
                        ...
                        while rowj < len(abnormal_wtgs_record):
                            if (datetime.datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj],"%Y-%m-%d %H:%M:%S") - datetime.datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj - 1],"%Y-%m-%d %H:%M:%S")).seconds == 60:
                                ...
                                if rowj == len(abnormal_wtgs_record):
                                    ...
                                    continue
                            elif (datetime.datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj - 1],"%Y-%m-%d %H:%M:%S") - datetime.datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowi],"%Y-%m-%d %H:%M:%S")).seconds >= MEAN*60:#10min
        ...
        def export(self):
            if len(self.abnormal_records) > 0:
                ...
                pd.io.sql.to_sql(self.abnormal_detail, 'GenWindingTemp', con=conn, if_exists='append')
                conn.close()
                try:
        ...
        def query_temp_mins_value(self, path, q1, q2, q3, q4):
            ...
            sqlstr = "SELECT wtid,real_time," + self.U_temp_tag + ',' +self.V_temp_tag + ','+ self.W_temp_tag + ','+self.power_tag+" FROM " + path['TABLE_NAME'] + \
                     " WHERE " \
                     + self.power_tag + " is not Null and " + \
                     self.U_temp_tag + " is not Null and " + \
                     self.V_temp_tag + " is not Null and " + \
                     self.W_temp_tag + " is not Null and " + \
            ...
            for id in range(len(timestampstr)):
                ...
                if len(res[(res['real_time'] >= fromtime) & (res['real_time'] <= totime)])>0:
                    ...
                    if len(res[(res['real_time'] >= fromtime) & (res['real_time'] <= totime)][self.U_temp_tag]) > 0:
                        ...
                        data = data.dropna(axis=0, how='any').tolist()
                        Uvalue.append(sum(data) / len(data))
                    else:
                        Uvalue.append(0)
                    if len(res[(res['real_time'] >= fromtime) & (res['real_time'] <= totime)][self.V_temp_tag]) > 0:
                        ...
                        data = data.dropna(axis=0, how='any').tolist()
                        Vvalue.append(sum(data) / len(data))
                    else:
                        Vvalue.append(0)
                    if len(res[(res['real_time'] >= fromtime) & (res['real_time'] <= totime)][self.W_temp_tag]) > 0:
                        data = res[(res['real_time'] >= fromtime) & (res['real_time'] <= totime)][self.W_temp_tag]
                        data = data.dropna(axis=0, how='any').tolist()
                        Wvalue.append(sum(data) / len(data))
                    else:
                        Wvalue.append(0)
                else:
                    powervalue.append(0)
            ...
            Uvalue = pd.DataFrame(Uvalue, index=timestampstr, columns=[path['WTGS_ID']])
            Vvalue = pd.DataFrame(Vvalue, index=timestampstr, columns=[path['WTGS_ID']])
            Wvalue = pd.DataFrame(Wvalue, index=timestampstr, columns=[path['WTGS_ID']])
            q1.put(powervalue)
            q2.put(Uvalue)
            q3.put(Vvalue)
            q4.put(Wvalue)
...
class query:
    ...
    def curve(self,abnormal_record):
        ...
        (conn,cur)=sqlite_conn()
        sqlstr = 'SELECT * FROM GenWindingTemp WHERE wtgs_id=\''+abnormal_record['wtgs_id'].iloc[0]+'\' AND abnormal_time BETWEEN \'' + abnormal_record['abnormal_start_time'].iloc[0] + '\' AND \'' + abnormal_record['abnormal_end_time'].iloc[0]+ '\''
        # print(sqlstr)
        res = pd.read_sql(sqlstr, con=conn)
        res.index=[datetime.datetime.strptime(timestr,'%Y-%m-%d %H:%M:%S') for timestr in res['abnormal_time'].tolist()]
        res['gen_winding_temperature'].plot()
        res['avg_winding_temperature'].plot()
        res['avg_power'].plot()
        plt.legend(loc='best')
        plt.grid()
