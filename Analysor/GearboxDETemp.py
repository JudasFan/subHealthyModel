...
class generate:
    ...
    class warning:
        ...
        def run(self):
            ...
            AbnormalRecord = pd.DataFrame(AbnormalRecord,columns=['farm_code', 'farm_name', 'wtgs_id', 'model_name', 'abnormal_time','gearbox_DE_temperature', 'avg_temperature', 'avg_power'])
...
class query:
    ...
    def detail(self,abnormal_record):
        ...
        # print(abnormal_record['farm_name'].iloc[0])
        # print(self.cal_farm.wtgs_path)
    ...
    def curve(self,abnormal_record):
        ...
        (conn,cur)=sqlite_conn()
        sqlstr = 'SELECT * FROM GearboxDETemp WHERE wtgs_id=\''+abnormal_record['wtgs_id'].iloc[0]+'\' AND abnormal_time BETWEEN \'' + abnormal_record['abnormal_start_time'].iloc[0] + '\' AND \'' + abnormal_record['abnormal_end_time'].iloc[0]+ '\''
        res = pd.read_sql(sqlstr, con=conn)
        ...
        res['gearbox_DE_temperature'].plot(ax=ax1,color='r')
        res['avg_temperature'].plot(ax=ax1)
        res['avg_power'].plot(ax=ax2,color='g')
        ax1.legend(['gearbox_DE_temperature','avg_temperature'], loc='upper left')
                    ...
                    if len(wtgsgroup) >= 4:
                        ...
                        for abwtgs in temp_value_group.index:
                            if temp_value_group.ix[abwtgs] >= max_value and max_value != 0:
                                ...
                                continue
            # filter the orignal abnormal record with 10 mins, if a wtgs is under abnormal status more than 10 mins, which selected
            AbnormalRecord = pd.DataFrame(AbnormalRecord,columns=['farm_code', 'farm_name', 'wtgs_id', 'model_name', 'abnormal_time','gearbox_DE_temperature', 'avg_temperature', 'avg_power'])
            SelectedAbnormalRecord = pd.DataFrame()
            abnormal_wtgs_list = sorted(list(set(AbnormalRecord['wtgs_id'].tolist())))
            for abnormal_wtgs in abnormal_wtgs_list: # loop all possible wtgs
                abnormal_wtgs_record = AbnormalRecord[AbnormalRecord['wtgs_id'] == abnormal_wtgs]
                if len(abnormal_wtgs_record) >= 600:
                    rowi = 0
                    while rowi < len(abnormal_wtgs_record):
                        print('rowi',rowi)
                        rowj = rowi + 1
                        if rowj == len(abnormal_wtgs_record):
                            break
                        while rowj < len(abnormal_wtgs_record):
                            print(abnormal_wtgs_record['abnormal_time'].iloc[rowj-1],abnormal_wtgs_record['abnormal_time'].iloc[rowj],(datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj],"%Y-%m-%d %H:%M:%S") - datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj - 1],"%Y-%m-%d %H:%M:%S")).seconds)
                            if (datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj],"%Y-%m-%d %H:%M:%S") - datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj - 1],"%Y-%m-%d %H:%M:%S")).seconds <=60:
                                rowj += 1
                                if rowj == len(abnormal_wtgs_record):
                                    if SelectedAbnormalRecord.empty:
                                        SelectedAbnormalRecord = abnormal_wtgs_record.iloc[rowi:rowj]
                                        self.abnormal_records.append(abnormalRecordMeta(self.wtgs_path,abnormal_wtgs_record,rowi,rowj))
                                    else:
                                        SelectedAbnormalRecord = pd.concat([SelectedAbnormalRecord, abnormal_wtgs_record.iloc[rowi:rowj]])
                                        self.abnormal_records.append(abnormalRecordMeta(self.wtgs_path, abnormal_wtgs_record, rowi, rowj))
                                    rowi = len(abnormal_wtgs_record)
                                    ...
                                    continue
                            elif (datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowj - 1],"%Y-%m-%d %H:%M:%S") - datetime.strptime(abnormal_wtgs_record['abnormal_time'].iloc[rowi],"%Y-%m-%d %H:%M:%S")).seconds >= 600:
                                if SelectedAbnormalRecord.empty:
                                    SelectedAbnormalRecord = abnormal_wtgs_record.iloc[rowi:rowj]
                                    self.abnormal_records.append(abnormalRecordMeta(self.wtgs_path, abnormal_wtgs_record, rowi, rowj))
                                    rowi = rowj
...
if __name__=="__main__":
    ...
    aaa=query('2017-12-06 00:00:00','2017-12-13 00:00:00','刘利军')
    res=aaa.abnormal_records()
    for i in range(len(res)):
        print(res.ix[i:i+1])
        aaa.detail(res.ix[i:i+1])
